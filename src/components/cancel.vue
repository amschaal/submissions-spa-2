<template>
  <div><q-btn icon="warning" label="Cancel Submission" title="Cancel submission" @click="cancel" v-if="!submission.cancelled && (!submission.locked || $store.getters.isStaff)" color="negative"/><q-btn label="Uncancel" title="Uncancel submission" @click="uncancel" v-if="submission.cancelled && $store.getters.isStaff" color="positive"/></div>
</template>
