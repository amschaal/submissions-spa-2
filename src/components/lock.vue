<template>
  <div v-if="$store.getters.isStaff"><q-btn label="Unlocked" title="Click to lock" icon="lock_open" @click="lock" v-if="!submission.locked" color="positive"/><q-btn label="Locked" title="Click to unlock" icon="lock" @click="unlock" v-if="submission.locked" color="negative"/></div>
</template>
