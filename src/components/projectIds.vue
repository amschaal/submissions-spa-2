...
export default {
  ...
  methods: {
    ...
    save (obj) {
      const self = this
      const action = !obj.id ? 'post' : 'put'
      const url = obj.id ? `/api/project_ids/${obj.id}/` : '/api/project_ids/'
    ...
    deleteProjectID (obj) {
      ...
      const self = this
      function remove () {
        const i = self.project_ids.indexOf(obj)
    ...
    generate_id (obj) {
      const n = obj.next_id + ''
      const suffix = n.length >= obj.num_digits ? n : new Array(obj.num_digits - n.length + 1).join('0') + n
